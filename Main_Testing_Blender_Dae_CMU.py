...
import numpy as np
from Lib.Dae_Import import AnimatedDAE_CMU #this seems to work for animations downloaded from mixamo
...
filename = './example_dae/01_02.dae'
all_files = glob.glob('C:\\Users\\asligar\\OneDrive - ANSYS, Inc\\Documents\\Scripting\\RFSX_2\\cad_library\\actors\\CMU_Database\\*.daez')
...
def main(filename,fps,smoothing=False,remove_hands=True,aedt_version= "2021.2"):
    ...
    #import mesh
    ani = AnimatedDAE_CMU(filename,save_path='./meshes/')
    ...
    if create_AEDT_proj:
        ...
        with Hfss(projectname=aedt.project_name,
            ...
            for node_id in meshes_dict:
                ...
                low_legs = ['RightKnee','RightAnkle','LeftKnee','LeftAnkle']
                ...
                if node_id_str in larms:
                    aedt.rotate(imported_names,single_rot=-90,axis='Z',reference_cs=cs_name)
                    aedt.rotate(imported_names,single_rot=-90,axis='Y',reference_cs=cs_name)
